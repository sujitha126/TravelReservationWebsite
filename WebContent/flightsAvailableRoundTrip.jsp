...
<body>
	<form method="post" action="TripSummaryController">
		...
		<table cellpadding="10">
			<c:set var="returningflightlist"
				value="${allflights['ReturningFlightList']}" />
			<tr>
				<td>
					<table border="1">
						<tr>
							<td>
								<table cellpadding="10">
									<tr>
										<td colspan="4" align="center">Returning Flight List</td>
									</tr>
									<c:forEach items="${returningflightlist}" var="flight">
										<tr>
											<td>
												<table cellpadding="10">
													<tr>
														<td rowspan="4"><input type="radio"
															value="${flight.flight_id}" name="flightforreturning"></td>
														<c:set var="flightselected" value="${flight}"
															scope="session" />
														<td>Flight Number: <c:out
																value="${flight.flight_num}" /></td>
